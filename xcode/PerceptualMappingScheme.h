...
//            std::cout << std::round(moods.getA << std::endl ;
            newMood = std::max( moods.getAvg(), minMood);
        }
        else
        {
            
             newMood = std::max( moods.getAverageOverWindow(windowsz, seconds), minMood );
...
//        std::cout << "New mood is: " << moods.getAverageOverWindow(windowsz, seconds) << std::endl ;
        return std::min(newMood, maxMood) ;

...
// Dynamic window sizes based on actual sample rate
// These will be calculated in the constructor
#define windowSizeA_DEFAULT 5.12
#define windowSizeB_DEFAULT 50
#define windowSizeC_DEFAULT 256
#define waitBetweenPeaks_DEFAULT 17.07
...
//check if I still use this class -- 10/1/205
class Stationary : public SignalAnalysis
{
public:
    Stationary( int id1, std::string portz1, SignalAnalysis *s  ) : SignalAnalysis( s, windowSizeC_DEFAULT )
    {
        stationary = false;
        ID1 = id1;
        port = portz1;
...
//        
//        msg.append(ID1);
//        msg.append(atoi(port.c_str()));
//        
//        msg.append( stationary ); // true if previous A block  was ’ peak ’ ( false = non peak )
...
//        msgs.push_back(msg);
    ...
    void update(float seconds) //try a low, then high pass filter
    {
        ...
        if( data1.size()<windowSizeA_DEFAULT ) return ; // don't bother until there is enough
...
protected:
    double actualSR;
    double windowSizeA, windowSizeB, windowSizeC;
    double waitBetweenPeaks;
...
StepDetection::StepDetection(int id1, std::string portz1, SignalAnalysis *s) : SignalAnalysis(s, windowSizeC_DEFAULT)
{
    // Initialize with default values
    actualSR = 51.2;
    windowSizeA = windowSizeA_DEFAULT;
    windowSizeB = windowSizeB_DEFAULT;
    windowSizeC = windowSizeC_DEFAULT;
    waitBetweenPeaks = waitBetweenPeaks_DEFAULT;
    
    // Try to get actual sample rate from sensor if available
    if (s != NULL) {
        // Cast to SensorData to get actual sample rate
        SensorData* sensorData = dynamic_cast<SensorData*>(s);
        if (sensorData != NULL) {
            double detectedSR = sensorData->getActualSampleRate();
            if (detectedSR > 0) {
                actualSR = detectedSR;
                updateWindowSizes();
            }
...
//TODO: calibrate more
void StepDetection::updateWindowSizes()
{
//OLD VERSION!!!
// Make these even smaller for faster response
//    windowSizeA = actualSR / 20.0;       // 0.05 second (was 0.1)
//    windowSizeB = actualSR / 2.0;        // 0.5 second (was 1.0)
//    windowSizeC = actualSR * 2.5;        // 2.5 seconds (was 5.0)
//    waitBetweenPeaks = actualSR / 5.0;   // 0.2 second (was 0.33)
    
    // Make these even smaller for faster response
    windowSizeA = actualSR / 40.0;       // 0.05 second (was 0.1)
    windowSizeB = actualSR / 4.0;        // 0.5 second (was 1.0)
    windowSizeC = actualSR * 1.125;        // 2.5 seconds (was 5.0)
    waitBetweenPeaks = actualSR / 2.5;   // 0.2 second (was 0.33)
    
    std::cout << "Updated window sizes for SR=" << actualSR << "Hz: "
              << "A=" << windowSizeA << ", B=" << windowSizeB
              << ", C=" << windowSizeC << ", wait=" << waitBetweenPeaks << std::endl;
}
...
{
    SignalAnalysis::update(seconds);
    if( data1.size() < windowSizeA ) return ;  // Use dynamic windowSizeA
...
//    
//    msg.append(ID1);
//    msg.append(atoi(port1.c_str()));
//    
//    msg.append( xMove );
//    msg.append( xSwing ); // true if previous A block was was ’ swing ’ phase ( false = stance )
//    msg.append( xPeak ); // true if previous A block  was ’ peak ’ ( false = non peak )
//    
//    msg.append( yMove ); // true if previous B block was ’moving’ phase ( false = stop )
//    msg.append( ySwing ); // true if previous A block was was ’ swing ’ phase ( false = stance )
//    msg.append( yPeak ); // true if previous A block  was ’ peak ’ ( false = non peak )
//    
//    msg.append( zMove ); // true if previous B block was ’moving’ phase ( false = stop )
//    msg.append( zSwing ); // true if previous A block was was ’ swing ’ phase ( false = stance )
//    msg.append( zPeak ); // true if previous A block  was ’ peak ’ ( false = non peak )
//
//    msg.append( combinedPeak ); // true if previous A block  was ’ peak ’ ( false = non peak )
...
public:
    SnapPeaks( int id1, std::string portz1, SignalAnalysis *s, BeatTiming *bt=NULL, bool onbeat = false) : StepDetection( id1, portz1, s )
    {
        ...
        // Update window sizes based on actual sample rate
        updateWindowSizes();
        
        beatTimer = bt;
        peakTimer.setWaitTime( beatTimer->getPulse8()*2.0 ); //for space of a 8th note don't recognize another
        ...
        //the thresholds at which a step is 
        xThresh = 0.75;
        yThresh = 1.0;
        zThresh = 0.4;
    ...
    bool findIC(int dataIndex, double &curpeak, double thresh=0.75, int takeDerivative=1 )
    {
        ...
        for( int i=0; i<xminIndex.size(); i++ )
        {
            ...
            if(!FoundPeakX) FoundPeakX = (xminIndex[i] >= snapSample+4) && (xmin[i] > thresh * 0.18 ); // reduced thresh
...
//        }
    ...
    void setMinDurationBetweenPeaks(float secs)
    {
        peakTimer.setWaitTime(secs);
    }
...
public:
    ...
    void setMinDurationBetweenSteps(float secs)
    {
        leftFoot->setMinDurationBetweenPeaks(secs);
        rightFoot->setMinDurationBetweenPeaks(secs);
    }
...
//        msg.append(rightID);
//        
//        msg.append( getStepCount() );
//        msg.append( isLeftOnset() );
//        msg.append( isRightOnset() );
//        msg.append( isRightOnset() || isLeftOnset() );
//        msg.append(curPeak);
//        msg.append(timeSinceLastStep);
//
//
//        
//        msgs.push_back(msg);

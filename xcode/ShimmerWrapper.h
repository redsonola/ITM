...
public:
    ...
    virtual void setLooping(float loop)
    {
        looping = loop;
    };
	virtual void stop()
	...
	virtual inline bool started(){ return (index != -1) && !done(); };
	virtual inline bool done(){ return index >= sData.size(); };
    virtual inline void setTimeModifierNum(float m){ modifier_num = m; };
...
std::vector<ci::osc::Message> ShimmerWrapper::getOSC()
{
	...
	for (int k = 0; k < howManyTimes; k++)
	{
		...
		for (int j = 0; j < SHIMMER_NUM_ARGS; j++)
		{
			msg.append(curData[i]->getData(j));
		}
...
std::vector<ci::osc::Message> ShimmerWrapper::getOSCScalarData(int whichData)
{
	...
	for (int k = 0; k < howManyTimes; k++)
	{
		for (i = k*MAX_MSG; i < MAX_MSG*(k + 1); i++)
		{
			...
			msg.append(whichData - 1 + _ID*(SHIMMER_NUM_ARGS-1)); // which input bus on supercollider
			msg.append(curData[i]->getData(whichData));
			msgVector.push_back(msg);
//            std::cout << "which bus #" << whichData - 1 + _ID*(SHIMMER_NUM_ARGS-1) << std::endl;
	...
	if (leftover > 0)
	{
		for (i = howManyTimes*MAX_MSG; i < curData.size(); i++)
		{
			ci::osc::Message msg;
			msg.setAddress(BUS_OSC_ADDR);
			msg.append(whichData - 1 + _ID*(SHIMMER_NUM_ARGS-1));
			msg.append(curData[i]->getData(whichData)); // which input bus on supercollider
			msgVector.push_back(msg);
            std::cout << "which bus #" << whichData - 1 + _ID*(SHIMMER_NUM_ARGS-1) << std::endl;
		}
	}

...
//        mData.push_back( crosscovar_avg ); //testing 8-5-2016
        ...
        mWeights.push_back(0.65);
        mWeights.push_back(0.35);
//        mWeights.push_back(0.5);
...
//        minVar = 100000;
    ...
    {
        // 7/2/2019 - note: this window may be much too short for social tango...
        const int WINDOW_FOR_USE_AS_MOTION_DATA = 2; //TODO: add class var & constructore -- //was 1.5 
...
//        std::cout << "Windowed variance: " << mData[0]->asFloat() << std::endl;
//        std::cout << "Step num: " << mData[1]->asFloat() << std::endl;
    };
    
    
    virtual double findMood()
    {
        double  m = PerceptualEvent::findMoodfixed();
...
public:
    enum Factors{ STEP_NUM=0, WINVARDIFF=1, PEAK_SIZE=2 };
     ...
     PointyVsSmoothInstrumentEvent(BeatTiming *timer, MotionAnalysisEvent *stepNum,  MotionAnalysisEvent *winVarDiff, MotionAnalysisEvent *peakSize) : OrchestralEvent(timer)
     {
         ...
         //change from 1 to 3 FOR NOW
         setMinMaxMood(0.0f, 1.0f);
         
         //add weights to those factors
         mWeights.push_back(0.0*1.0); //CDB --changing so as to reduce the effect of busy movement on this
         mWeights.push_back(0.65*1.0);
         mWeights.push_back(0.35*1.0);

...
//        virtual double getAvgWindowedVarDiff()=0;
    ...
    public:
        ...
        enum SongIDs{ FRAGMENTS = 0, POR_UNA_CABEZA = 1, LA_CUMPARSITA = 2, EL_CHOCLO = 3 };
...
//        OrchThickVsThinEvent *ot;
    ...
    public:
        ...
        void addSensorBodyPart(int idz, SensorData *sensor, std::vector<UGEN * > *ugens, BodyPart whichBody )
        {
            ...
            WindowedVariance *variance = new WindowedVariance(filter15, SR / 4, idz, sensor->getPort() ) ;
...
//            TangoInstruments instruments;
            ...
            std::vector<SendSignal *> sendSignalsFollower;
            std::vector<SendSignal *> sendSignalsLeader;
    ...
    public:
        ...
        std::vector<SendSignal *> getSendSignalsFollower(){ return sendSignalsFollower; };
        std::vector<SendSignal *> getSendSignalsLeader(){ return sendSignalsLeader; };
...
//                outfile  << motionAnalysisParamsStrings[i] << ",";
        ...
        Pareja(Dancer *l, Dancer *f, std::vector<UGEN * > *ugens, BeatTiming *timer, bool recordData=false)
            {
//                melody = NULL;
//                accompaniment = NULL;
...
//                winVarDiffFollower->setMinMax(0, 0.05);
                ...
                //DISABLING FOR NOW SINCE DOES NOT WORK WITH BUSY/SPARSE HAVING A DIFFERENT MAX YET 8/27- TODO:Fix
...
//                ugens->push_back(pv);
                ...
                //hack hack
                sendSignalsFollower.push_back((SendSignal *) sigs[1]);
                sendSignalsFollower.push_back((SendSignal *) sigs[3]);
                
                sendSignalsLeader.push_back((SendSignal *) sigs[0]);
                sendSignalsLeader.push_back((SendSignal *) sigs[2]);
...
//                
//                ugens->push_back( optapLeader ); //see above but disabled - 6/28/2019 enabling -- //disabling 9/2019
...
//                mMappingSchemas.push_back( (MappingSchema *) optapLeader); //see above, but disabled
//                mMappingSchemas.push_back((MappingSchema *)  optapFollower);
                ...
                for(int i=0; i<4; i++)
                {
                    ...
                    mMappingSchemas.push_back((MappingSchema *)  sigs[i]);
                }
    ...
    public:
        ...
        void changeBusySparseScale(WhichDancerType which, float newScale)
        {
            ...
            std::cout << DancerTypeStr(which) << " busy/sparse scale changed to: " << newScale << std::endl;

        };
        void changeStaccatoLegatoInstrumentScale(WhichDancerType which, float newScale)
        {
            MappingSchema *mappingSchema;
            
            if( which == WhichDancerType::LEADER1 )
            {
                mappingSchema = couples[0]->getMappingSchema(INSTRUMENT_POINTROUND_LEADER);
            }
            else if ( which == WhichDancerType::FOLLOWER1 )
            {
                mappingSchema = couples[0]->getMappingSchema(INSTRUMENT_POINTROUND_FOLLOWER);
            }
            else if( which == WhichDancerType::LEADER2 && curMapping == CurrentMapping::TWO_COUPLES )
            {
                mappingSchema = couples[1]->getMappingSchema(BUSY_SPARSE_LEADER);
            ...
            std::cout << DancerTypeStr(which) << " staccato/legato scale changed to: " << newScale << std::endl;
...
//            }
        ...
        virtual void mappingForTwoCouples()
        {
            mMappingSchemas.clear(); //clear previous mapping
            initMappingVector();
...
//                player.addOrnament( leaderornL );
        ...
        virtual void loadSong(BeatTiming *timer)
        {

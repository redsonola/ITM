...
#define INDEX_FROM_MELODY_END_COUNTS_AS_LAST 4
    ...
    public:
        ...
        MidiNote(MidiNote *note)
        {
            ...
            channel = note->channel; //if -1 use default
            oneOftheLastMelodyNotes = note->oneOftheLastMelodyNotes;
        }
        double tick;
        double absTick;
        double duration;
        NoteDuration noteDuration; //for mapping in ITM
        int    pitch;
        int velocity;
        ...
        {
            std::stringstream ss;
            ss << "MidiNote: pitch: " << pitch << " velocity: " << velocity << " tick: " << tick << " channel: " << channel << std::endl;
            return ss.str(); 
        }
    };
...
//        std::vector<MidiNote> mel = getMelody(track);
        ...
        bool  found = false;
        MidiNote *note = NULL;
        while(!found && i < getMelody(track).size() )
        {
            found  = melody[track][i].absTick >= tick;
            i++;
        }
        if(found)
        {
            note = &melody[track][i-1];
        }
        return note;
    }
...
//        std::cout << "track: " << track << " tickStart: " << tickStart << " tickEnd: " << tickEnd << " indexOf " << indexOfLastChecked << std::endl;
        ...
        int i = indexOfLastChecked;
        bool found = false;
        while(i < getMelody(track).size() && !found )
        {
//            std::cout << accompFile[i].pitch << " " << accompFile[i].tick << " " << accompFile[i].absTick << "\n";

            try{
                found = melody[track][i].absTick >= tickEnd;
                if(!found && melody[track][i].absTick >= tickStart)
                {
                    accomp.push_back(&melody[track][i]);
                }
                i++;
            }
            catch(...)
            {
                ...
                std::cout << "i: " << i << " accompFile size " << melody[track].size() << std::endl;
                exit(1);
            }
        }
        return accomp;
...
public:
 ...
 //   MidiOutUtility(std::string portname = "IAC Bus 1")
    ...
    {
//                std::cout << "note: " << note->pitch << "," << note->channel << std::endl;
    ...
    void send(MidiControlMessage *msg)
    {
//        std::cout << "channel: " << msg->channel << "," << "control: " << msg->control << "," << msg->value << std::endl;

        //    inline MidiMessage MakeControlChange(uint8_t channel, uint8_t control, uint8_t value)

        midiout->send(mm::MakeControlChange(msg->channel, msg->control, msg->value));
    }

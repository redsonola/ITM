...
//            std::cout << "checking if need to play?  beats: " << beatsPlayed << std::endl ;
    ...
    //not yet used
    virtual SoundFile *getNextStructuredFile(std::shared_ptr<std::vector<int>> profile, std::shared_ptr<std::vector<int>> hsprofile, std::shared_ptr<std::vector<int>> instrumentProfile = NULL)
    {
        return NULL;
    
    };
    
    virtual SoundFile *getCurFile(std::shared_ptr<std::vector<int>> hsprofile, float seconds = 0) //if a counter melody, needs to be sync'd with main melody
    {
        return NULL;
    };
...
//            std::shared_ptr<std::vector<int>> profile = curSoundFile->getProfile();
            
            //for now message - MUST FIX
            ci::osc::Message msg;
            msg.setAddress(addr);
            msg.append(curSoundFile->getTrackNumber()-1);
            msg.append(curSoundFile->getClipNumber()-1);
            msg.append(curSoundFile->getAbletonID());
            msg.append(sectionType); //melody, accomp., etc.?
            msg.append(curSoundFile->getHarmonySectionProfile()->at(0)); //get section of song
            msg.append(curSoundFile->getHarmonySectionProfile()->at(1)); //get harmony of song
...
public:
    ...
    virtual SoundFile *getNextStructuredFile(std::shared_ptr<std::vector<int>> profile, std::shared_ptr<std::vector<int>> hsprofile, std::shared_ptr<std::vector<float>> instrumentProfile)
    {
...
//            }
    ...
    {
        // build mood profile
        std::shared_ptr<std::vector<int>> profile(new std::vector<int>) ;
...
//                std::cout << "in Curfile :"<< ((PerceptualEvent * )mMappingSchemas[i])->getName() << ":  " << moodNum << "     " << seconds << "  \n";
    ...
    int getBusySparse(float seconds, float event_mapping_window=0.5)
    {
        int moodNum ;
        ...
        for(int i=0; i<mMappingSchemas.size(); i++)
        {
            if( !isOrchestration(mMappingSchemas[i]) )
            {
                if ( mMappingSchemas[i]->getMappingType() == MappingSchema::MappingSchemaType::EVENT )
                {
                    moodNum = std::round(( (PerceptualEvent * )mMappingSchemas[i] )->getCurMood(event_mapping_window, seconds));
                }
            }
        }
        return moodNum;
...
public:
    MelodySection( BeatTiming *timer, FootOnset *onset, Instruments *ins, float perWindowSize=5 ) : MusicSection( timer, ins )
    {
        ...
        //TODO: check
        mappingPerceptionWindowForMelody = 0.5;// perWindowSize; //default is 0.5 second...
    ...
    virtual SoundFile *getCurFile(std::shared_ptr<std::vector<int>> hsprofile, float seconds = 0) //if a counter melody, needs to be sync'd with main melody
    {
        ...
        std::shared_ptr<std::vector<int>> profile(new std::vector<int>);
        std::shared_ptr<std::vector<int>> instrumentProfile(new std::vector<int>);

        //TODO: added busy v sparse
//        profile->push_back(1);
        for(int i=0; i<mMappingSchemas.size(); i++)
        {
            int moodNum =  std::round(((PerceptualEvent * )mMappingSchemas[i])->getCurMood(mappingPerceptionWindowForMelody, seconds)) ;
//            std::cout << "in Curfile :"<< ((PerceptualEvent * )mMappingSchemas[i])->getName() << ":  " << moodNum << "     " << seconds << "  \n";
    ...
    virtual std::shared_ptr<std::vector<int>> getHarmonySectionProfile()
    {
        if ( curSoundFile != NULL )
            return curSoundFile->getHarmonySectionProfile();
        else
        {
            return std::shared_ptr<std::vector<int>>(NULL);
        }
    };
    ...
    virtual std::vector<ci::osc::Message> getOSC()
    {
        ...
        if(_COUT_DEBUG_)
            if(msgs.size() > 0)
                std::cout << msgs[0].getAddress() << ","  << msgs[0].getArgInt32(0) << "," << msgs[0].getArgInt32(1) << "," << msgs[0].getArgInt32(2) << std::endl;
        return msgs;
    ...
    int findBusySparse()
    {
        ...
        for(int i=0; i<mMappingSchemas.size(); i++)
        {
            int moodNum =  std::round(((PerceptualEvent * )mMappingSchemas[i])->getCurMood(mappingPerceptionWindowForMelody, curSeconds) );
            //            std::cout << "in Curfile :"<< ((PerceptualEvent * )mMappingSchemas[i])->getName() << ":  " << moodNum << "     " << seconds << "  \n";
...
public:
    ...
    virtual SoundFile *getCurFile(std::shared_ptr<std::vector<int>> hsprofile, Orchestra *orchestration, float seconds = 0) //if a counter melody, needs to be sync'd with main melody
    {
        // build mood profile
        std::shared_ptr<std::vector<int>> profile(new std::vector<int>);
    ...
    void determineOrchestration(Orchestra *orchestration)
    {
        ...
        if(justStartedPlaying || curOrch.size() <= 0)
        {
            ...
            std::shared_ptr<std::vector<float>> profile(new std::vector<float>);
    ...
    virtual void update(std::shared_ptr<std::vector<int>> hsprofile, Orchestra *orchestration, float seconds = 0)
    {
    ...
    virtual SoundFile *getNextStructuredFile(std::shared_ptr<std::vector<int>> profile, std::shared_ptr<std::vector<int>> hsprofile, Orchestra *orchestration)
    {
...
//            {
//                std::cout << "In Player: " << CIRCLING_VOLUME << gmsgs[i].getArgFloat(0) << std::endl;
//            }
        }
...
//            std::cout << CREATE_BOLEO_ORNAMENT << "," << 1699 << std::endl;
            ...
            msg.append( curOrch.getInstrViaIndex(0)->getTrackID() ); //this orch is from main melody, which should have 1 instrument only --> this sends the correct track to ableton

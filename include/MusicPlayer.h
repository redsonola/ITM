...
public:
    ...
    virtual SoundFile *getNextStructuredFile(boost::shared_ptr<std::vector<int>> profile, boost::shared_ptr<std::vector<int>> hsprofile, boost::shared_ptr<std::vector<float>> instrumentProfile)
    {
...
//            }
    ...
    virtual SoundFile *getCurFile(boost::shared_ptr<std::vector<int>> hsprofile, float seconds = 0) //if a counter melody, needs to be sync'd with main melody
    {
        ...
        boost::shared_ptr<std::vector<float>> instrumentProfile(new std::vector<float>) ;
...
public:
    ...
    void determineOrchestration(Orchestra *orchestration)
    {
        ...
        if(justStartedPlaying || curOrch.size() <= 0)
        {
            ...
            boost::shared_ptr<std::vector<float>> profile(new std::vector<float>);

...
//                << ",";
                ...
                for(int i=0; i<4; i++)
                {
                    msg.append( busySparseMIDINumeratorNorm[busySparse-1] + (std::rand()/(10.0*double(RAND_MAX))));
...
//        if( phraseWaiting ) phraseWaiting = melody->afterEndOfPhrase();
    ...
    virtual std::vector<ci::osc::Message> getOSC()
    {
        ...
        if( shouldSend )
        {
            ...
            msg.append(int(_id));
            for(int i=0; i<5; i++)
                msg.append( f->getMotionData().at(i)->scaledValue());
...
//    
    ...
    virtual std::vector<ci::osc::Message> getOSC()
    {
        ...
        msg.setAddress(ss.str());
//        msg.append(_ID); //send ID -- this is really ID of the user of this mapping, eg. which dancer or couple --> 0 is sent to everything
        msg.append(float(getCurMood()));
...
//        std::cout << ss.str() << ": " << getCurMood() << std::endl;
    ...
    public:
        ...
        {
            std::cout << "DEBUG: Adding pareja " << parejaCount << " - mData.size() before: " << mData.size() << std::endl;
            ...
            std::cout << "DEBUG: After adding pareja " << (parejaCount-1) << " - mData.size() after: " << mData.size() << std::endl;
            std::cout << "DEBUG: Expected size should be: " << (parejaCount * locationOfCrossCoVarAndNumberOfFactorsPerCouple) << std::endl;
        }
        ...
        std::vector<double> avgAcrossDancers()
        {
            ...
            std::cout << "DEBUG: mData.size() = " << mData.size() << std::endl;
            std::cout << "DEBUG: locationOfCrossCoVarAndNumberOfFactorsPerCouple = " << locationOfCrossCoVarAndNumberOfFactorsPerCouple << std::endl;
            std::cout << "DEBUG: parejaCount = " << parejaCount << std::endl;
            std::cout << "DEBUG: mData.size() % locationOfCrossCoVarAndNumberOfFactorsPerCouple = " << (mData.size() % locationOfCrossCoVarAndNumberOfFactorsPerCouple) << std::endl;
            
            if (mData.size() % locationOfCrossCoVarAndNumberOfFactorsPerCouple != 0) {
                std::cout << "ERROR: Data size mismatch! Expected multiple of " << locationOfCrossCoVarAndNumberOfFactorsPerCouple 
                         << " but got " << mData.size() << " elements." << std::endl;
                std::cout << "This suggests data is being added incorrectly for real sensors vs iPhone app." << std::endl;
            }
            
            // Fixed assertion: check that remainder is 0 (data size is perfect multiple)
            assert( mData.size() % locationOfCrossCoVarAndNumberOfFactorsPerCouple == 0 );
...
//    };
    ...
    protected:
        ...
        bool turnOffAllButEnergy; //yep. doing this.

    public:
        SendSignal( Filter *signal, int did = 0, std::string oMsg = SEND_BACK,  int srDiv = 6, int controlNumber=6) : ContinousOutMappingSchema(did)
        {
            filteredSignal  = (Filter *) signal;
            filteredSignal->calcMotionData(true);
            _id = did;
            
            turnOffAllButEnergy = true; //just setting this flag here. sorry.
        ...
        void setMidiMappingMode(int map)
        {
            midiMappingMode = map;
        }
        ...
        void changeChannel(std::vector<int> ch)
        {
            midiChannel = ch;
        }
        
        void setStartControlNum(int ctrl)
        {
            midiControl = ctrl;
        }
        
        int getNumberOfMidiControlsNeeded()
        {
            return (energy->valuesForOSC().size() / 3); // + accelEnergy->valuesForOSC().size();
        }
        
        //no longer send OSC, send MIDI
...
//                std::vector<float> accelVals = accelEnergy->valuesForOSC();
                ...
                int mControl = midiControl;
//                std::cout << "----midiControl: start: " << midiControl << std::endl;

                if( midiMappingMode == -1 )
                {
...
//                    std::cout << "control: " << i << "," << val << std::endl
                ...
                {
                    //only sends one message for mapping to ableton.
                    MidiControlMessage * msg = outputSignalPowerMessage((midiMappingMode-midiControl)*3, midiMappingMode, energyVals );
                    msgs.push_back(msg);
        ...
        MidiControlMessage * outputMessage(int i, int control, std::vector<float> vals)
        {
            
            float MIDI_MAX_VAL = 127;

            MidiControlMessage *msg = new MidiControlMessage();
            msg->control = control;
            msg->channel = midiChannel[0];
            msg->value = std::round(float(vals[i]) * float(MIDI_MAX_VAL)* 75.0f);
        
            //make sure it doesn't go over min or max midi values
            if(msg->value > float(MIDI_MAX_VAL))
                msg->value = MIDI_MAX_VAL;
        ...
        MidiControlMessage * outputSignalPowerMessage(int i, int control, std::vector<float> vals)
        {
            
            //combines 1st 3 values into 1
            //i is where to start
...
//            std::cout << "vals: " << v << std::endl;
        ...
        //not sending OSC bc sending midi
        virtual std::vector<ci::osc::Message> getOSC()
        {
...
//                std::vector<float> accelVals = accelEnergy->valuesForOSC();
//
////                std::cout << "updating send signal" << oscMessage << "id: " << _id <<"\n";
////                for(int i=0; i<energyVals.size(); i++)
////                    std::cout << float(energyVals[i]) << ",";
////                std::cout << "\n----------------------\n";
//
...
////                    msg.append( slopeVals[i] );
//
//                for(int i=0; i<energyVals.size(); i++)
//                    msg.append( float(energyVals[i]) );
//
//                for(int i=0; i<accelVals.size(); i++)
//                    msg.append( float(accelVals[i]) );
//
//                msgs.push_back(msg);
//
//            }

...
class InteractiveTangoReadFromAndroidApp : public App {
...
public:
    ...
    InteractiveTangoReadFromAndroidApp();
    ~InteractiveTangoReadFromAndroidApp();
...
InteractiveTangoReadFromAndroidApp::InteractiveTangoReadFromAndroidApp() :
...
void InteractiveTangoReadFromAndroidApp::onSendError( asio::error_code error )
{
    if( error ) {
...
void InteractiveTangoReadFromAndroidApp::sendOSCInputsToWekinator(std::vector<ci::osc::Message> msgs, float seconds, int index )
{
    ...
    {
        send->send(msgs[i], std::bind( &InteractiveTangoReadFromAndroidApp::onSendError,
...
std::vector<ci::osc::Message> InteractiveTangoReadFromAndroidApp::collectMessagesforDifferentPorts(std::string addr, std::vector<ci::osc::Message> *msgs, std::vector<ci::osc::Message> nmsgs)
{
...
std::vector<ci::osc::Message> InteractiveTangoReadFromAndroidApp::collectMessagesforDifferentPortsBack(std::vector<ci::osc::Message> *msgs)
{
...
void InteractiveTangoReadFromAndroidApp::sendOSCMessages( std::vector<ci::osc::Message> msgs, float seconds )
{
...
//    }
    ...
    if( shouldSend )
    {
        for(int i=0; i<msgs.size(); i++)
        {
            if(send != NULL) send->send(msgs[i], std::bind( &InteractiveTangoReadFromAndroidApp::onSendError,
                                                           this, std::placeholders::_1 ));
            
            //send the remote laptop as well.
//            remoteLaptopSender.send(msgs[i], std::bind( &InteractiveTangoReadFromAndroidApp::onSendError, this, std::placeholders::_1 ));
...
//            }
        ...
        {
            ornamentSender.send(ornament_msgs[i], std::bind( &InteractiveTangoReadFromAndroidApp::onSendError,
                                                            this, std::placeholders::_1 ));
        }
        for(int i=0; i<circling_msgs.size(); i++)
        {
            circlingSender.send(circling_msgs[i], std::bind( &InteractiveTangoReadFromAndroidApp::onSendError,
                                                            this, std::placeholders::_1 ));
        }
...
//        {
//            backSender.send(back_msgs[i], std::bind( &InteractiveTangoReadFromAndroidApp::onSendError,
//                                                    this, std::placeholders::_1 ));
//
...
//        }
        ...
        {
            reverbSender.send(reverb_msgs[i], std::bind( &InteractiveTangoReadFromAndroidApp::onSendError,
                                                        this, std::placeholders::_1 ));
        }
...
void InteractiveTangoReadFromAndroidApp::loadPorUnaCabezaStaccatoSavedFiles()
{
    //load sensors
...
void InteractiveTangoReadFromAndroidApp::loadFragmentsStaccatoSavedFiles()
{
    //load sensors
...
void InteractiveTangoReadFromAndroidApp::loadChosenFiles()
{
    //load sensors
...
void InteractiveTangoReadFromAndroidApp::loadPorUnaCabezaNormSavedFiles()
{
    //load sensors
...
//int whichDTWWekinatorOutput;
//int selectedWekinatorSenderIndex;
int InteractiveTangoReadFromAndroidApp::addSendtoWekinator()
{
...
void InteractiveTangoReadFromAndroidApp::setupSensorsForTest()
{
...
void InteractiveTangoReadFromAndroidApp::setupSensorsForTestTwoDancers()
{
    setupSensorsForTest();
...
void InteractiveTangoReadFromAndroidApp::clearListeningUDPSocket()
{
...
void InteractiveTangoReadFromAndroidApp::setupSenderUdp(osc::SenderUdp *sender)
{
    try {
...
void InteractiveTangoReadFromAndroidApp::setup()
{
//setup to save OSC
#ifdef SAVE_ALL_OSC
...
void InteractiveTangoReadFromAndroidApp::prepareSettings(Settings *settings)
{
...
void InteractiveTangoReadFromAndroidApp::mouseDown( MouseEvent event )
{
    
}

void InteractiveTangoReadFromAndroidApp::sendPhotoSync(float seconds)
{
//send sync message to a machine recording mocap video, so that sensor & video are sync'd
    ...
    msg.append(seconds);
    syncSender.send(msg, std::bind( &InteractiveTangoReadFromAndroidApp::onSendError,
                                   this, std::placeholders::_1 ));
}

void InteractiveTangoReadFromAndroidApp::handleOutputWekOSC(osc::Message message, float seconds)
{
...
//void InteractiveTangoReadFromAndroidApp::clearResidualData()
//{
...
//                std::cout << dID << "," << whichShimmer << " quat: " << quat[0] << " , " << quat[1] << " , " << quat[2]  << " , " <<  quat[3] << std::endl ;
            ...
            {
                int whichSignal = message.getArgInt32(3);
                int indexStart = 0;
...
void InteractiveTangoReadFromAndroidApp::printWekModeMenu()
{
...
void InteractiveTangoReadFromAndroidApp::printNormalWiiMenu()
{
    std::cout <<" CURRENT WII MENU \n";
...
void InteractiveTangoReadFromAndroidApp::printChangingParamsMenu()
{
    std::cout <<" CURRENT WII MENU \n";
...
bool InteractiveTangoReadFromAndroidApp::isWiiButtonDown(std::string addr, osc::Message message)
{
...
void InteractiveTangoReadFromAndroidApp::changeMovementParamsWithWii(osc::Message message)
{
...
void InteractiveTangoReadFromAndroidApp::handleWiiOSC(osc::Message message)
{
...
void InteractiveTangoReadFromAndroidApp::addMarkerToSensorFile(std::string marker)
{
...
void InteractiveTangoReadFromAndroidApp::wiiToWekinator(osc::Message message)
{
    osc::Message wek_msg;
    bool send = false;
...
#ifdef WEKINATOR_ENABLED
    if (send)
        wiiWekinatorSenders[selectedWekinatorSenderIndex]->send(wek_msg, std::bind( &InteractiveTangoReadFromAndroidApp::onSendError,
#endif                                                                              this, std::placeholders::_1 ));
}
                                                                                   
void InteractiveTangoReadFromAndroidApp::sendMidiMessages( std::vector<InteractiveTango::MidiMessage *> msgs)
{
    for(int i=0; i<msgs.size(); i++)
    {
...
void InteractiveTangoReadFromAndroidApp::updateSignalTree(float seconds, float pastSeconds)
{
    //update this first
    if( shouldSendOSC )
    {
...
void InteractiveTangoReadFromAndroidApp::update()
{
    setpriority(PRIO_PROCESS, 0, 1);
...
void InteractiveTangoReadFromAndroidApp::stopStartOSCToAbleton()
{
...
void InteractiveTangoReadFromAndroidApp::startOSCToAbleton()
{
    shouldSendOSC = true;
...
void InteractiveTangoReadFromAndroidApp::stopOSCToAbleton()
{
    shouldSendOSC = false;
    std::cout << "Stopped sending OSC\n";
    restartSong();
...
void InteractiveTangoReadFromAndroidApp::restartSong()
{
    //restart the song!
...
void InteractiveTangoReadFromAndroidApp::keyDown( KeyEvent event )
{
    if(midiMappingMode)
    {
        if(event.getChar()=='m')
...
void InteractiveTangoReadFromAndroidApp::addTestDancersNoSensors()
{
...
void InteractiveTangoReadFromAndroidApp::changeSensorColor()
{
    if( mSensorData.size() <= 0) return;
...
void InteractiveTangoReadFromAndroidApp::addPareja()
{
    //add a pareja
...
void InteractiveTangoReadFromAndroidApp::addDancer()
{
...
//InteractiveTango::Pareja::SongIDs whichSongIsPlaying;
void InteractiveTangoReadFromAndroidApp::cycleThroughPlaylist()
{
...
void InteractiveTangoReadFromAndroidApp::playPorUnaCabeza()
{
...
void InteractiveTangoReadFromAndroidApp::playElChoclo()
{
...
void InteractiveTangoReadFromAndroidApp::playLaCumparsita()
{
...
void InteractiveTangoReadFromAndroidApp::playFragments()
{
...
//cout for now
void InteractiveTangoReadFromAndroidApp::printMenu()
{


}

ci::ColorA InteractiveTangoReadFromAndroidApp::generateRandomColor()
{
...
void InteractiveTangoReadFromAndroidApp::drawGrid(float size, float step)
{
...
void InteractiveTangoReadFromAndroidApp::screenshot()
{
...
void InteractiveTangoReadFromAndroidApp::draw()
{
...
SensorData * InteractiveTangoReadFromAndroidApp::getSensor(string deviceID, int which)
{
    bool found = false;
    int index = 0;
...
SensorData *InteractiveTangoReadFromAndroidApp::getSensor(std::string whichShimmer)
{
...
void InteractiveTangoReadFromAndroidApp::addLiveDancer(SensorData *sensor, int sid)
{
...
void InteractiveTangoReadFromAndroidApp::addLivePareja(std::map<int, int> &l, std::map<int, int> &f, int lID, int fID)
{
...
void InteractiveTangoReadFromAndroidApp::saveAllSensors()
{
...
void InteractiveTangoReadFromAndroidApp::parseFilenameForInfo(string fname, string *deviceID, int *whichID)
{
    *deviceID = fname.substr(fname.find('.') + 1, fname.find('_') - 1);
    *whichID = std::atoi( fname.substr(fname.find('_') + 2, fname.find('.') - 1 ).c_str());
}

void InteractiveTangoReadFromAndroidApp::loadSensor(std::string input)
{
	VisualizeShimmer *visualizer = new VisualizeShimmer();
	SensorData *sensor;
    int whichShimmer;
...
CINDER_APP( InteractiveTangoReadFromAndroidApp, RendererGl,
           []( InteractiveTangoReadFromAndroidApp::Settings *settings ) //note: this part is to fix the display after updating OS X 1/15/18
           {
